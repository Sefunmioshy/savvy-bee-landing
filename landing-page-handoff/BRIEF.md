...
**FINAL landing page:** `Landing.dc.html`

**NOT IN SCOPE** (separate pages, linked from the landing page but not part of this rebuild):
- `About.dc.html` — team / where we've been page
- `Legal.dc.html` — terms, privacy, fees, complaints (tabbed)
- `Investors.dc.html` — investor page, exists but is **unlinked** from the landing page
- `Savvy Bee Prototype.html` — the in-app demo, loaded in an iframe by the landing page (see §7)
- `Home.dc.html`, `Spend.dc.html`, `Save.dc.html`, and all other `*.dc.html` app screens — these are the mobile app, not the landing page
...
| 4 | `About` | nav link | `About.dc.html` | No |
...
| 26 | `About` | footer COMPANY | `About.dc.html` | No |
...
**Iframe fit behaviour (lives in `Savvy Bee Prototype.html`, not the landing page).** The prototype shell injects a style + script into every screen it loads:
...
| The demo iframe | Loads `Savvy Bee Prototype.html#Home.dc.html` as a same-origin relative path. Must be deployed alongside the landing page, or replaced with a hosted URL. |
...
**Legal / About**
18. `About.dc.html` and `Legal.dc.html` are linked from the landing page nav and footer. Are they in scope for the rebuild, or should those links point elsewhere for now? (They are listed as NOT IN SCOPE above.)
19. The Legal page's fee figures (transfer tiers ₦10/₦25/₦50, card funding 1.4% + ₦100, brokerage 1.00%, FX rate + 0.5%) were set as reasonable defaults, not confirmed economics. Needs sign-off before publishing.
