...
## Scope: three pages plus the demo

| Build | File | Spec |
|---|---|---|
| **The landing page** — the primary rebuild | `Landing.dc.html` | BRIEF.md §1–§11 |
...
| **The demo** — ship as-is, do **not** rebuild | `Savvy Bee Prototype.html` | BRIEF.md §14 |

Legal and About are linked from the landing page's nav and footer, so all three
ship together. They are static and share the landing page's tokens exactly —
no new colours, no new type styles. Legal is the only one with behaviour: a
four-tab strip that reads and writes the URL hash, which is how the landing
footer's `#terms` / `#privacy` / `#fees` / `#complaints` links land on the
right document.

**The demo is not a rebuild target.** `Savvy Bee Prototype.html` is a
self-contained 2.3 MB file with 46 app screens inlined; the landing page loads
it in an iframe. Copy it into the build output and point the iframe at it. It
already opens on the Home screen and already contains the fit/overscroll fix
that makes it work in a phone-sized iframe — BRIEF.md §14 explains both, and
why that fix must travel with the file if you ever host it elsewhere.
...
| `Legal.dc.html` | Legal page — four tabbed documents. In scope. |
| `About.dc.html` | About page — team and track record. In scope. |
| `Savvy Bee Prototype.html` | The demo, self-contained. **Ship as-is, do not rebuild.** |
| `assets/` | The three hero-phone PNGs plus the two portraits and five photos About uses |
...
Out of scope: `Investors.dc.html` (deliberately unlinked), and the 46 individual app screens — those live inside the demo file and are not web pages. See BRIEF.md "SCOPE".
...
4. **No page title, meta description, OG tags, favicon or `lang` attribute** — on any of the three pages.

Two more that need sign-off before Legal can be published: the fee figures are
reasonable defaults rather than confirmed economics, and bracketed placeholders
in the legal copy still need counsel. The page carries a visible draft banner
saying so; the `draftNotice` prop turns it off.
